...
-- one-to-one relationship between restaurant and address table.
...
-- many-to-many relationship & third cross-reference table categories_dishes
...
--Import of projectdata.sql to populate schema data tables.
...
--Query to display restaurant details
SELECT name, street_name, street_number, telephone
FROM restaurant, address;

--Query to get the best rating 
SELECT MAX(rating) AS best_rating FROM review;

-- Query to display name, price, category sorted by dish name
...
-- Query to display results sorted by category name.
...
-- Query to display all the spicy dishes, their prices and category.
...
--Query that incorporates multiple tables that display the dish name as dish_name and dish count as dish_count.
...
SELECT rating, description FROM review WHERE rating = (SELECT MAX(rating)
FROM review);
